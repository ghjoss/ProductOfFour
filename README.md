...
and one is a simple text file. The file names start with "Inc_" and denote the difference. Thus for a difference 
(increment) of 1, the files are Inc_1.csv and Inc_1.txt.
...
Notes:
	...
	o	In the notes that follow, the resultant number is referred to as R. It may also be
		referred to as R[n] where it is the result based upon a starting number n.
	...
	o	When looking in squaresX.txt (X=1,2,3,...), the R's formed by starting integer(n)=1, difference(k)=1 is not formed by any other 
		n:k combination. The same is true for n=2:k=2, n=3:k=3, ... n=10,k=10.  But n=11:k=11 is also formed by n=4:k=19 and n=19:k=4. The second report
		in BigDictionary.txt (search for the word "Report") shows all of the n:k combinations which also include n=k. There it can be more easily
		seen which results have n=k and also have n <> k.
